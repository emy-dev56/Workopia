<x-layout>
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                @auth
                    ...
                    <div x-data="{ open: false }">
                        ...
                        <div x-show="open" x-cloak="true"
                            class="fixed  justify-center flex items-center inset-0 w-full h-full bg-gray-900/50">
                            <div @click.away="open=false"class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
                                <form action="{{ route('applicant.store', $job->id) }}" enctype="multipart/form-data"
                                    ...
                                    <button type="submit"
                                        class="text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium cursor-pointer text-indigo-700 bg-indigo-100 hover:bg-indigo-200">Submit
                                        Application</button>
                                    <button @click="open=false" type="button"
                                        class="text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium cursor-pointer text-black bg-gray-200 hover:bg-gray-400">Cancel</button>
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            @endif
            @guest
                <p class="text-center text-sm color-gray-500">
...
<script>
    async function loadLocation() {
        ...
        const address_details = "{{ $job->city . ', ' . $job->state }}";

        const response = await fetch('/geolocation?address=' + encodeURIComponent(address_details));
